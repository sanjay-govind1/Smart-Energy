...
class Command(BaseCommand):
    help = "Update ML model by combining dummy data with real readings"
    ...
    def handle(self, *args, **kwargs):
        ...
        self.stdout.write(self.style.SUCCESS(
            f" Combined dataset saved: {COMBINED_CSV}\n"
            f"   → Dummy records: {dummy_count}, Real records: {real_count}, Total: {len(combined_df)}"
        ))
        ...
        if len(X) > 5:  # ensure enough data for train/test
            ...
            self.stdout.write(self.style.SUCCESS(
                f" Model trained. R² score: {score:.3f}"
            ))
        else:
            model = RandomForestRegressor(n_estimators=100, random_state=42)
            model.fit(X, y)
            self.stdout.write(self.style.WARNING(
                " Not enough data for proper train/test split, trained on all data."
            ))

        # ----- Step 6: Save model -----
        joblib.dump(model, MODEL_PATH)
        self.stdout.write(self.style.SUCCESS(
            f" Updated model saved: {MODEL_PATH}"
        ))
